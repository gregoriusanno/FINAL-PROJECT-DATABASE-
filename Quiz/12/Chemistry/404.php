...
session_start();
 ...
 $sql = "SELECT course.CourseName, course.CourseName, chapter.ChapterName, chapter.page FROM course
    JOIN chapter ON (course.CourseID = chapter.CourseID)
    JOIN sesi ON (chapter.ChapterID = sesi.ChapterID)
    WHERE chapter.ChapterID = 2";
    
 $sql2 = "SELECT COUNT(*) c FROM quizhighscore WHERE QuizID = 2";
...
<body>
    ...
    <!-- 404 Start -->
        ...
        <div class="container">
            ...
            for($i = 1 ; $i <= 5 ; $i++){
                require_once 'connection.php';
                $q = "select * from quizquestion where QuizID = 2 AND QuizQuestionID = $i"; 
                $query = mysqli_query($conn,$q);
                ?>

                <?php
                $_SESSION["quiz"] = $chapter['ChapterID'];
                ...
                <div class="py-2">
                        <h1 class="btn-lg-crcl px-5"><li class="upscale-20 text-dark"><?php echo $rows["Question"];?></h1>
                            <ol type="A">
                                    ...
                                    <?php
                                    $q = "select * from quizanswer where QuizID = 2 AND quizQuestionID = $i"; 
                                    $query = mysqli_query($conn,$q);
                                    while($rows = mysqli_fetch_array($query) ){
                                        ?>
        ...
        <div class="container">
        <a class="btn btn-white btn-lg-refin-l upscale-30 px-4 py-1" href="../<?php echo $return["page"];?>" >< Return</a>
            <p data-inline="true" align="right"></p>
                <input type="submit" name="submit" Value="Submit >" class="btn btn-lg-refin-r m-auto d-block" /> <br> 
                    <!-- <a class="btn btn-white btn-lg-refin-r upscale-30 px-4 py-1" href="../../../score.php">Finish ></a> -->
            </p>
        </div>
    </form>
    ...
    <!-- Footer Start -->
    <div class="container-fluid bg-dark text-light footer pt-5 mt-7 wow fadeIn" data-wow-delay="0.1s">
        <div class="container py-5">
            <div class="row g-5">
